...
객체지향 5대원칙이란 유지보수와 확장성이 높은 프로그램을 개발하기 위해 만들어놓은 원칙을 의미한다.

1. Single Responsibility Principle (단일 책임 원칙)

- 소프트웨어의 설계 부품(클래스,함수)는 단 하나의 책임을 가져야 한다.

2. Open-Closed Principle (개방-폐쇄 원칙)

- 기존의 코드를 변경(Closed)하지 않고 기능을 추가하거나 수정 할 수 있도록(Open) 설계해야 한다.
...
- DIP를 만족한다는 것은 의존관계를 맺을 때 구체적인 클래스보다는 인터페이스나 추상클래스와 관계를 맺는다는 것을 의미한다.
